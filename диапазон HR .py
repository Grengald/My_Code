...
file_path = 'C:\\Users\\Grengald\\Desktop\\Котинг\\ИКИ РАН\\Sav bd\\GRB130609B.sav'
...
N_GRB = 0
GRB_Names = ['GRB130609B']
SJDs_Max = [4453.39532259]
T90s = [24.46]
...
# Короткий профиль
ax1.bar(JD_Short_datetime, Short_Profile, width=width, edgecolor='green', facecolor='none', label='Short Profile')
ax1.errorbar(JD_Short_datetime, Short_Profile, yerr=Short_Profile_Err, fmt='.', color='green')
...
Short_Profile_in = np.arange(130, 157)
Short_Profile_in = np.array(Short_Profile_in)
JD_Short_datetime = np.array(JD_Short_datetime)
Short_Profile = np.array(Short_Profile)
ax1.bar(JD_Short_datetime[Short_Profile_in], Short_Profile[Short_Profile_in], width=width, edgecolor='red', facecolor='red', label='')
...
# Изменяем NN_HR, чтобы он содержал индексы бинов с максимальной SHR
NN_HR = np.arange(152, 157)

# Обновляем второй график с использованием SHR
ax2.plot([JD_Long_1_datetime[i] for i in NN_HR], HR[NN_HR], '+', markersize=5, label='Hardness Ratio')
ax2.errorbar([JD_Long_1_datetime[i] for i in NN_HR], abs(HR[NN_HR]), yerr=HR_Err[NN_HR], color='blue', fmt='+')
ax2.axvline(jd_to_datetime(JD_Long_1[N_Short_Profile_Max]), linestyle='--', color='red')
