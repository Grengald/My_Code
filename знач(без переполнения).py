...
file_path = 'C:\\Users\\Grengald\\Desktop\\Котинг\\ИКИ РАН\\Sav bd\\GRB100707A.sav'
...
N_GRB = 0
GRB_Names = ['GRB100707A']
SJDs_Max = [3384.5428568300]
T90s = [40.0]
...
while True:
    if n <= len(Left_Short_Profile) and (m > len(Right_Short_Profile) or Left_Short_Profile[-n] > Right_Short_Profile[m-1]):
        ...
        n += 1
    elif m <= len(Right_Short_Profile):
...
max_shr_idx = np.argmax(SHR)
max_shr_value = SHR[max_shr_idx]
...
# Вертикальная линия для максимальной SHR
plt.axvline(x=max_shr_idx + 1, linestyle='--', color='red', label=f'Max SHR = {max_shr_value:.2f}')
plt.legend()
