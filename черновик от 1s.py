...
file_path = 'C:\\Users\\Grengald\\Desktop\\Котинг\\ИКИ РАН\\Sav bd\\GRB021008A.sav'
...
N_GRB = 0
GRB_Names = ['GRB021008A']
SJDs_Max = [555.7816101600]
T90s = [17.2]
...
width = 0.0000116  # ширина столбцов гистограммы

# Длинный профиль
ax1.bar(JD_Long_1, CR_Long_1, width=width, color='blue', alpha=0.6, label='Long Profile')
ax1.errorbar(JD_Long_1, CR_Long_1, yerr=CR_Long_1_Err, fmt='.', color='blue')
ax1.plot(JD_Long_1, BG_Long, color='#969696', label='Background Long', linestyle='--')

# Короткий профиль
ax1.bar(JD_Short, Short_Profile*5, width=width, color='green', alpha=0.6, label='Short Profile x5')
ax1.errorbar(JD_Short, Short_Profile*5, yerr=Short_Profile_Err*5, fmt='.', color='green')
ax1.plot(JD_Short, BG_Short*5, color='#FFD700', label='Background Short x5', linestyle='--')

# Вертикальная линия для максимальной счетной скорости
ax1.axvline(JD_Long_1[N_CR_Long_1_Max], linestyle='--', color='red', label='Max Count Rate')
...
print('JD_Long_1[NN_HR]:', JD_Long_1[NN_HR])
print('JD_Short[NN_HR]:', JD_Short[NN_HR])
print('Signal_Long_1[NN_HR]:', Signal_Long_1[NN_HR])
print('Signal_Short[NN_HR]:', Signal_Short[NN_HR])

print('Short_Profile_Err[NN_HR]:', Short_Profile_Err[NN_HR])
print('CR_Long_1_Err[NN_HR]:', CR_Long_1_Err[NN_HR])

# Нижний график: отношение жесткости
ax2.plot(JD_Long_1[NN_HR], HR[NN_HR], '+', markersize=5, label='Harness Ratio')
ax2.errorbar(JD_Long_1[NN_HR], HR[NN_HR], yerr=HR_Err[NN_HR],  color='blue', fmt='+')
ax2.axvline(JD_Long_1[N_CR_Long_1_Max], linestyle='--', color='red')
